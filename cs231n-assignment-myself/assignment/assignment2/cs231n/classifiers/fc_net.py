...
#         pass
        ...
        dx2, dw2, db2 = affine_backward(dx, s2_cache)
        grads['W2'] = dw2 + self.reg * self.params['W2']
        grads['b2'] = db2
        dx1, dw1, db1 = affine_relu_backward(dx2,s1_cache)
...
#         pass
        for i in range(self.num_layers-1):
            ...
            if i==0:
                self.params['W%s' % (i+1)] = np.random.normal(0,scale = weight_scale,  size = (input_dim,hidden_dims[i]))
                self.params['b%s' % (i+1)] = np.zeros(hidden_dims[i])
            else:
                self.params['W%s' % (i+1)] = np.random.normal(0,scale = weight_scale,  size = (hidden_dims[i-1],hidden_dims[i]))
                self.params['b%s' % (i+1)] = np.zeros(hidden_dims[i])
        self.params['W%s' % (self.num_layers)] = np.random.normal(0,scale = weight_scale,  size = (hidden_dims[i],num_classes))
