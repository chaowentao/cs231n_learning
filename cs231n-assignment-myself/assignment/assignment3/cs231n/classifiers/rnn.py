...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        # pass features (N,D) W_proj (D,H) captions (N,T) W_embed (V,W) Wh(H,H) Wx (W,H) W_vocab(H,V)

        h0, affine_cache = affine_forward(features, W_proj, b_proj)  # (N, H)
        x, embed_cache = word_embedding_forward(captions_in, W_embed) #(N,T,W)
        if self.cell_type == 'rnn':
            h, rnn_cache = rnn_forward(x, h0, Wx, Wh, b) # (N,T,H)
        elif self.cell_type == 'lstm':
            h, lstm_cache = lstm_forward(x, h0, Wx, Wh, b)
        out, temp_affine_cache = temporal_affine_forward(h, W_vocab, b_vocab) # (N,T,V)
